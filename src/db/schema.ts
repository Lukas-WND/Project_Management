...
export const usersToProjects = t.pgTable("users_to_projects", {
  userId: t.integer("user_id").references(() => users.id).notNull(),
  projectId: t.integer("project_id").references(() => projects.id).notNull(),
}, (table) => [
		t.primaryKey({ columns: [table.userId, table.projectId] })
	],
);
